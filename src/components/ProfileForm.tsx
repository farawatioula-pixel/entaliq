...
export default function ProfileForm({ profile }: { profile: Profile }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-10 space-y-10">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-neutral-300">About you</label>
        <textarea
          rows={4}
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-neutral-300">Track</label>
            ...
            <button
              ...
              className={`rounded-sm border px-4 py-3 text-left text-sm transition-colors ${
                category === c
                  ...
                  : "border-line text-neutral-400 hover:text-fg"
              }`}
            >
              {categoryLabel[c]}
            </button>
      ...
      <div>
        <div className="flex items-center justify-between">
          <label className="block text-sm font-medium text-neutral-300">Your services</label>
          <button
            type="button"
            onClick={addService}
            ...
            <div key={i} className="rounded-sm border border-line bg-surface p-5">
              <div className="grid gap-4 sm:grid-cols-[2fr_1fr]">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-sm border border-line bg-ink px-3 py-2.5 text-[15px] text-fg focus:border-red focus:outline-none"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-neutral-400">
              ...
              {services.length > 1 && (
                <button
                  ...
                  className="mt-3 text-xs font-semibold text-neutral-400 hover:text-red"
                >
                  Remove this service
                </button>
              )}
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-line pt-8">
        ...
        <button
          ...
          className="text-sm font-semibold text-neutral-400 hover:text-fg"
        >
          Sign out
        </button>
      </div>
