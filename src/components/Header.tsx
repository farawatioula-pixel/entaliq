...
const links = [
  ...
  { href: "/directory", label: "Marketplace" },
...
export default function Header() {
  ...
  const accountHref = user ? "/profile" : "/signup";
  const accountLabel = user ? "My Profile" : "Become a Seller";
    ...
    <header className="sticky top-0 z-50 border-b border-line bg-ink/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
        ...
        <div className="hidden items-center gap-4 lg:flex">
          ...
          <Link
            href={accountHref}
            className="inline-flex items-center rounded-sm border border-cyan px-5 py-2 text-[15px] font-semibold text-cyan transition-colors hover:bg-cyan hover:text-ink"
          >
            {accountLabel}
          </Link>
          <Link
        ...
        >
          <ul className="flex flex-col gap-1">
            ...
            <li>
              <Link
                href={accountHref}
                className="block rounded-sm border border-cyan px-2 py-3 text-center text-base font-semibold text-cyan"
              >
                {accountLabel}
              </Link>
            </li>
            {!user && (
              <li>
                <Link
                  href="/login"
                  className="block rounded-sm px-2 py-3 text-center text-base font-medium text-neutral-300"
                >
                  Log in
                </Link>
              </li>
